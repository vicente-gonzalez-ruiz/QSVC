...
##############################	##############################
    ...
    # cp /home/cmaturana/QSVC/research/paper0/gpt/data/mobile.5trl_16_42000_8GOPs__transcode_gops/info_snrFrames_GOPs info_snrFrames_GOPs_A
    cp ../info_snrFrames_GOPs_A .

    ###############################################
    # Search slope in MJ2K by distortion in MCJ2K #
    ###############################################
    mcj2k searchSlope_byDistortion_j2k --GOPs=$GOPs --TRLs=$TRLs --SRLs=$SRLs --nLayers=$Ncapas_T --pixels_in_x=$RES_X --pixels_in_y=$RES_Y --distortions=info_snrFrames_GOPs_A
