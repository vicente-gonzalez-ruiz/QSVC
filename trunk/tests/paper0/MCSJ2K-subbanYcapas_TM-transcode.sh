...
SRLs=`echo "$TRLs-1" | bc -l` #`echo "$available_SRLs-$discard_SRLs_Tex" | bc` # Nº de planos de bits que se envian realmente (+ 1, el signo)
...
Ncapas_M=1   # 1
algorithm="FS" # -1. "transcode" A direct transcode that use "combination".
...
slope=43000 # 42000 46000

#while [ $slope -ge 42000 ]; do           # 1 slope X punto. 1/4     0. Compress/expand. Slopes con/sin GAINS
...
##############################	##############################
    ...
    # TRANSCODE #
    #############
    #mcj2k transcode --algorithm=$algorithm --combination=$combination --update_factor=$update_factor --GOPs=$GOPs --TRLs=$TRLs --SRLs=$SRLs --FPS=$FPS --BRC=500000 --discard_SRLs=$discard_SRLs --pixels_in_x=$RES_X --pixels_in_y=$RES_Y --block_size=$block_size --search_range=$search_range --nLayers=$Ncapas_T
...
#    echo -e $rate'\t'$rmse'\t'$slope >> $data_base/info.$NAME.$TRLs""trl_$Ncapas_T"_"$GOPs"GOPs_upfactor"$upF""_
#    let slope=slope-256                   # 1 slope X punto. 3/4
