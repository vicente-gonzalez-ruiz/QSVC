...
ffmpeg -y -s ${X_DIM}x${Y_DIM} -i $DATA/$VIDEO.yuv -vcodec mpeg4 -g $GOP_SIZE -qscale $Q_SCALE -vframes $PICTURES 1.avi
echo "ffmpeg -y -s ${X_DIM}x${Y_DIM} -i $DATA/$VIDEO.yuv -vcodec mpeg4 -g $GOP_SIZE -qscale $Q_SCALE 1.avi" >> trace
