...
VIDEO=pedestrian_1920x1080x25x420x375.yuv
...
if [[ ! -e $DATA/$VIDEO.yuv ]]; then
    ...
    mencoder pedestrian_area_1080p25.y4m -ovc raw -of rawvideo -vf format=i420 -o $VIDEO.yuv
    cd $current_dir
fi
...
QSCALE=40
#QSCALE=40 -> rate_avc=2026

avc(){
rm -f $data_file

    x264 --keyint $GOPsize --crf $QSCALE --input-res ${X_DIM}x${Y_DIM} --fps $FPS -o $compressed_video $DATA/$VIDEO.yuv
...
#slope=44090 rate_mcj2k=2024.95486381
slope=44090
#while [ $slope -le 51 ]; do
    mcj2k compress --pictures=$PICTURES --pixels_in_x=${X_DIM} --pixels_in_y=${Y_DIM} --slopes=$slope --temporal_levels=$TRL
    rate_mcj2k=`mcj2k info --pictures=$PICTURES --pictures_per_second=$FPS --temporal_levels=$TRL | grep "Total average:" | cut -d " " -f 3`
    mkdir tmp
    cp *.mjc tmp
    cd tmp
    mcj2k expand --pictures=$PICTURES --pixels_in_x=${X_DIM} --pixels_in_y=${Y_DIM} --temporal_levels=$TRL
