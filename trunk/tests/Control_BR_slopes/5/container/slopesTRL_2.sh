...
carpeta="codestream_2/data-FD-MCJ2K-buscaRate_129-container_352x288x30x420x300.sh_44000.32.32.5.0.0.1.3"
carpeta2="codestream_2/data-FD-MCJ2K-buscaRate_129-container_352x288x30x420x300.sh_44000.32.32.1.0.0.1.3"
...
iteraciones=6
#divisores=(1.01	1.0075	1.005	1.0025	0.999	0.998	0.997	0.996	0.995	0.994	0.993	0.992	0.991	0.99	0.9875	0.985	0.9825	0.98)
#decrementores=(0.0005	0.001	0.002	0.003)
divisores=(1.01	1.0075	1.005	1.0025)
decrementores=(0.0005	0.001	0.002 0.003)
...
for L in "${slopes_L[@]}"; do
	...
	for divisor in "${divisores[@]}"; do
		...
		for decrementor in "${decrementores[@]}"; do
			...
			d=`echo "($divisor*1000)/1" | bc`	# Para la comparación con decimales.
			if [ $d -lt 1000 ] ; then	# Alejamos los valores, de uno. (restando)
			...
			else # Alejamos los valores, de uno. (sumando)				!!!!!
				divi1=`echo "$divisor+$decrementor" | bc -l`
				divi2=`echo "$divi1+$decrementor" | bc -l`
				divi3=`echo "$divi2+$decrementor" | bc -l`
				divi4=`echo "$divi3+$decrementor" | bc -l`
			fi
			# El más cercano a 1 es el divi1. Recta de números: diviN .. divi2 .. divi1 .. 1
			# Por lo que cambiaremos H4 según divi1.
