...
J2K () {
    ...
    # Compress
	(time mcj2k compress --slopes='"'$1'"' --pictures=$PICTURES --temporal_levels=$TEMPORAL_LEVELS --pixels_in_y=$PIXELS_IN_Y --pixels_in_x=$PIXELS_IN_X --block_size=$BLOCK_SIZE) 2>> $data_dir/TIMES/mcj2k_$2.dat

    rm -rf tmp ; mkdir tmp ; cp *.mjc *type* tmp

	# Extract
    (time mcj2k extract --temporal_levels=$TEMPORAL_LEVELS --pictures=$PICTURES --resolutions=$2) 2>> $data_dir/TIMES/extract_$2.dat
	...
	cd $data_dir/tmp	
	(time mcj2k expand --pictures=$PICTURES --temporal_levels=$TEMPORAL_LEVELS --pixels_in_y=$piy --pixels_in_x=$pix --subpixel_accuracy=$2 --block_size=$bs) 2>> $data_dir/TIMES/expand_$2.$pix""x$piy.dat
    ...
    rate=`mcj2k info --pictures=$PICTURES --temporal_levels=$TEMPORAL_LEVELS --pictures_per_second=$FPS | grep "Total average:" | cut -d " " -f 3`
