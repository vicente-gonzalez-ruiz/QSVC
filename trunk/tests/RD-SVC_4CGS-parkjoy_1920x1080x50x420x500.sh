...
while [ $kbps_corte -le $kbps_corte_max ]; do
	cd ..
	#Extrae la capa n
	BitStreamExtractorStatic $in/CGS_4layers.264 $out/extracted_e -e $E_PARAMETROS:$kbps_corte #> $out/infos/info_extracted$n

	cd $out
	#Reconstruye el .yuv
	H264AVCDecoderLibTestStatic extracted_e n.yuv #> infos/info_yuv$n
...
let kbps_corte=kbps_corte+150	#con 150 es suficiente para que salgan todas las capas.
done
